          ...
          <div className="space-y-2 max-h-72 overflow-y-auto pr-1">
            {loading ? (
              ...
              locations.map((loc) => {
                ...
                const count = loc._count?.assets ?? 0;
                    ...
                    {isEditing ? (
                      ...
                      <>
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          {/* Còn máy thì khoá luôn nút và nói rõ lý do — để người dùng bấm rồi
                              mới ăn 409 là bắt họ tự đoán vị trí nào đang vướng. */}
                          <button
                            type="button"
                            onClick={() => handleDelete(loc.id, loc.name)}
                            disabled={deletingId === loc.id || count > 0}
                            className="p-1.5 rounded-lg text-slate-400 hover:text-red-600 hover:bg-red-50 disabled:opacity-40 disabled:hover:text-slate-400 disabled:hover:bg-transparent dark:hover:bg-white/[0.06] transition-colors"
                            title={
                              count > 0
                                ? `Còn ${count} thiết bị ở đây. Chuyển chúng sang chỗ khác rồi mới xoá được.`
                                : 'Xoá vị trí'
                            }
                          >
