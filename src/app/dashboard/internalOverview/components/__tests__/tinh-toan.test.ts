import { CHI_SO, gopNenTang } from '../tinh-toan';
...
const nenTang = (p: Partial<ThongKeNenTang> = {}): ThongKeNenTang =>
  ({
    platform: 'facebook',
    views: 0,
    likes: 0,
...
describe('gopNenTang — cộng chỉ số kỳ này', () => {
  it('cộng đủ 5 chỉ số trên hàng thẻ đầu trang', () => {
    const ra = gopNenTang([
      nenTang({ views: 100, likes: 10, comments: 5, shares: 2, posts: 1 }),
      nenTang({ platform: 'tiktok', views: 900, likes: 90, comments: 45, shares: 18, posts: 9 }),
    ]);

    for (const { ma } of CHI_SO) {
  ...
  it('cộng riêng bộ số kỳ trước — đây là gốc của mũi tên tăng/giảm', () => {
    const ra = gopNenTang([
      nenTang({ views: 100, truoc: { views: 80, likes: 8, comments: 4, shares: 2, posts: 1 } }),
      nenTang({
        platform: 'tiktok',
        views: 200,
  ...
  it('cộng followers, so_kenh, tong_kenh', () => {
    const ra = gopNenTang([
      nenTang({ followers: 1000, so_kenh: 3, tong_kenh: 5 }),
      nenTang({ platform: 'youtube', followers: 250, so_kenh: 1, tong_kenh: 2 }),
    ]);
  ...
  it('một nền tảng thì giữ nguyên số, không nhân đôi', () => {
    const ra = gopNenTang([nenTang({ views: 42, posts: 7, followers: 9 })]);
    expect(ra).toMatchObject({ views: 42, posts: 7, followers: 9 });
  });
});

describe('gopNenTang — chuỗi theo ngày cho biểu đồ xu hướng', () => {
  /*
  ...
  it('cộng đúng ngày với ngày kể cả khi hai nền tảng lệch danh sách ngày', () => {
    const ra = gopNenTang([
      nenTang({ theo_ngay: [ngay('2026-08-01', 100), ngay('2026-08-02', 200)] }),
      nenTang({
        platform: 'tiktok',
  ...
  it('xếp ngày tăng dần dù đầu vào lộn xộn — biểu đồ vẽ theo đúng thứ tự mảng', () => {
    const ra = gopNenTang([
      nenTang({ theo_ngay: [ngay('2026-08-03', 3), ngay('2026-08-01', 1)] }),
      nenTang({ platform: 'tiktok', theo_ngay: [ngay('2026-08-02', 2)] }),
    ]);
  ...
  it('cộng cả 5 chỉ số trong chuỗi ngày, không riêng lượt xem', () => {
    ...
    const ra = gopNenTang([
      nenTang({ theo_ngay: [mot] }),
      nenTang({ platform: 'tiktok', theo_ngay: [mot] }),
    ]);
  ...
  it('không sửa dữ liệu đầu vào', () => {
    const goc = nenTang({ views: 100, theo_ngay: [ngay('2026-08-01', 100, 2)] });
    const banSao = JSON.parse(JSON.stringify(goc));

    gopNenTang([goc, nenTang({ platform: 'tiktok', views: 50 })]);
  ...
  it('gộp hai lần trên cùng đầu vào cho ra kết quả y hệt', () => {
    const list = [
      nenTang({ views: 100, theo_ngay: [ngay('2026-08-01', 100)] }),
      nenTang({ platform: 'tiktok', views: 50, theo_ngay: [ngay('2026-08-01', 50)] }),
    ];

    expect(gopNenTang(list)).toEqual(gopNenTang(list));
  });
});
