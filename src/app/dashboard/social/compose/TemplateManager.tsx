...
function FillVariablesForm({ template, onApply, onCancel }: {
  ...
  return (
    <div className="space-y-4">
      <div>
        <p className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">{t.templateManager.fillVariables}</p>
        <div className="space-y-2">
          {template.variables.map(v => (
            <div key={v}>
              ...
              <input
                type="text"
                placeholder={t.templateManager.enterVariable(v)}
                value={values[v] || ''}
      ...
      <div>
        <p className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">{t.templateManager.preview}</p>
...
export default function TemplateManager({ currentMessage, currentHashtags, onApply }: Props) {
  ...
  const handleSave = () => {
    if (!newName.trim()) return toast.error(t.templateManager.enterTemplateName);
    if (!currentMessage.trim()) return toast.error(t.templateManager.contentEmpty);
    ...
    toast.success(t.templateManager.templateSaved(tpl.name));
  };
  ...
  const handleDelete = (id: string) => {
    ...
    setTemplates(updated);
    saveTemplates(updated);
    toast.success(t.templateManager.templateDeleted);
  };
  ...
  const handleUseTemplate = (tpl: Template) => {
    if (tpl.variables.length > 0) {
      ...
      setShowPanel(false);
      toast.success(t.templateManager.templateApplied(tpl.name));
    }
  };
        ...
        {showPanel && (
          ...
          >
            <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100">
              <h3 className="text-sm font-black text-slate-800">📄 {t.templateManager.contentTemplate}</h3>
              ...
              {filling ? (
                <FillVariablesForm
                  ...
                  onApply={(text, tags) => {
                    ...
                    setFilling(null);
                    setShowPanel(false);
                    toast.success(t.templateManager.templateAppliedGeneric);
                  }}
                ...
                <>
                  ...
                  <div>
                    ...
                    >
                      ...
                      {t.templateManager.saveCurrentAsTemplate}
                    </button>
                      ...
                      {showSaveForm && (
                        ...
                        >
                          <div className="pt-2 space-y-2">
                            <input
                              type="text"
                              placeholder={t.templateManager.templateNamePlaceholder}
                              value={newName}
                            ...
                            {extractVariables(currentMessage).length > 0 && (
                              <p className="text-[10px] text-indigo-600 font-semibold">
                                {t.templateManager.detectedVariables(extractVariables(currentMessage).map(v => `{${v}}`).join(', '))}
                              </p>
                            )}
                  ...
                  {templates.length === 0 ? (
                    <div className="text-center py-8">
                      ...
                      <p className="text-sm font-bold text-slate-400">{t.templateManager.noTemplatesYet}</p>
                      <p className="text-xs text-slate-300 mt-1">{t.templateManager.noTemplatesHint}</p>
                    </div>
                  ) : (
                    <div className="space-y-2">
                      <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">{t.templateManager.templateListCount(templates.length)}</p>
                      {templates.map(tpl => (
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              {tpl.variables.length > 0 && (
                                <p className="text-[10px] text-indigo-500 font-semibold mt-0.5">
                                  {t.templateManager.variablesLabel(tpl.variables.map(v => `{${v}}`).join(', '))}
                                </p>
                              )}
