...
export default function TikTokChannelsPage() {
  ...
  const profilesQuery = useQuery({
    queryKey: ['owned-tiktok-profiles', page, debouncedSearch, sortBy],
    queryFn: () => token
      ? scraperService.getTiktokProfiles(token, { page, page_size: PAGE_SIZE, search: debouncedSearch || undefined, sort_by: sortBy, is_owned: true })
  ...
  const scrapeMutation = useMutation({
    mutationFn: (raw: string) => {
      ...
      const clean = raw.replace(/.*tiktok\.com\/@?/, '').replace(/^@/, '').split(/[/?]/)[0].trim();
  ...
  const toggleMutation = useMutation({
    mutationFn: ({ id, field }: { id: number; field: 'is_bookmarked' | 'is_tracked' }) => {
      if (!token) throw new Error('No token');
      return scraperService.toggleTiktokProfile(token, id, field);
    },
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['owned-tiktok-profiles'] }),
  });
  ...
  const videosQuery = useInfiniteQuery({
    queryKey: ['owned-tiktok-videos', debouncedVideoSearch, sortVideos, dateFrom, dateTo, minPlays, market, contentLine, channel, hashtag],
    queryFn: ({ pageParam = 1 }) => {
      if (!token) return Promise.reject('No token');
      return scraperService.getOwnedChannelVideos(token, {
        page: pageParam, page_size: 24, platform: 'tiktok',
  ...
  return (
    <div className="flex flex-col gap-5">
        ...
        <div className="flex items-center gap-3">
          <div className="relative flex-1 max-w-xl">
            ...
            <input
              ...
              placeholder="username hoặc link TikTok (vd: mixigaming)"
      ...
      <h2 className="text-sm font-semibold text-foreground">
        Kênh TikTok {total > 0 && <span className="font-normal text-slate-500">({total})</span>}
      </h2>
      ...
      {!profilesQuery.isLoading && profiles.length === 0 && (
        <div className="flex flex-col items-center py-16 gap-4 bg-card border border-border rounded-xl">
          ...
          <p className="text-sm text-foreground font-medium">Chưa có kênh TikTok nội bộ</p>
          <p className="text-xs text-slate-400 text-center max-w-sm">Nhập TikTok username ở trên để thêm kênh.</p>
        </div>
      )}
