...
import { CircleNotch, FilmReel, Warning, MagnifyingGlassPlus, VideoCamera, UserCircle, Sparkle, Timer, BookmarkSimple, MagnifyingGlass, X, ArrowsDownUp, Eye, Tag } from '@phosphor-icons/react';
...
import { scraperService, KuaishouProfile } from '@/services/scraperService';
...
export default function KuaishouExternalPage() {
  ...
  const [classifyingProfile, setClassifyingProfile] = useState<KuaishouProfile | null>(null);
  ...
  const hasFilters = !!debouncedFilter || !!filterKeyword || !!minViews || !!dateFrom || !!dateTo || sortBy !== 'scraped';
  const clearFilters = () => { setFilterSearch(''); setFilterKeyword(''); setMinViews(''); setDateFrom(''); setDateTo(''); setSortBy('scraped'); };
  ...
  const profilesQuery = useQuery({
    queryKey: ['kuaishou-profiles', profilePage, debouncedProfileSearch, profileSortBy, profileTab, channelTypeTab, selectedProductLine],
          ...
          <div className="flex flex-wrap items-center gap-2.5 bg-card border border-border rounded-xl p-3 shadow-xs">
            <div className="relative flex-1 min-w-[180px] max-w-sm">
              ...
              <input
                ...
                placeholder="Lọc theo mô tả, hashtag..."
        ...
        <>
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 border border-border rounded-xl p-4">
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              {canManageChannels && (
                <SyncAllChannelsButton platform="kuaishou" channelCount={profileTotal} />
              )}
            </div>
          </div>
            ...
            <div className="flex flex-col items-center py-16 gap-3 bg-card border border-border rounded-xl text-center">
              ...
              <p className="text-xs text-slate-400 max-w-sm">
                {profileTab === 'periodic'
                  ...
                  : 'Nhập User ID ở trên để bắt đầu cào.'}
              </p>
            </div>
          )}
