...
import {
  ArrowLeft, Users, Eye, Heart, ChatCircle, VideoCamera,
  ArrowsClockwise, BookmarkSimple, Timer, CircleNotch, FilmReel, PaperPlaneTilt,
} from '@phosphor-icons/react';
import toast from 'react-hot-toast';

import { useAuthStore } from '@/store/auth-store';
import { scraperService, KuaishouVideo, KuaishouProfile } from '@/services/scraperService';
...
export default function KuaishouProfileDetailPage() {
  ...
  const [sortBy, setSortBy] = useState<'scraped' | 'date' | 'views' | 'likes' | 'comments'>('scraped');
  ...
  const detailQuery = useQuery({
    queryKey: ['kuaishou-profile-detail', id],
    queryFn: () => token ? scraperService.getKuaishouProfileDetail(token, id) : Promise.reject(),
    enabled: !!token && !!id,
  ...
  const videosQuery = useInfiniteQuery({
    queryKey: ['kuaishou-profile-videos', id, debouncedSearch, minViews, sortBy],
    queryFn: ({ pageParam = 1 }) => {
      if (!token) return Promise.reject();
      return scraperService.getKuaishouProfileVideos(token, id, {
        page: pageParam,
        page_size: 24,
  ...
  useEffect(() => {
    if (prevProcessing.current && !isProcessingNow) {
      queryClient.invalidateQueries({ queryKey: ['kuaishou-profile-videos', id] });
      queryClient.invalidateQueries({ queryKey: ['kuaishou-profile-detail', id] });
  ...
  const toggleMutation = useMutation({
    mutationFn: (field: 'is_bookmarked' | 'is_tracked') =>
      token ? scraperService.toggleKuaishouProfile(token, id, field) : Promise.reject(),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['kuaishou-profile-detail', id] }),
  });
  ...
  return (
    <div className="flex flex-col gap-5">
          ...
          <div className="flex items-start gap-6">
            <div className="w-28 h-28 rounded-full bg-slate-100 dark:bg-slate-700 overflow-hidden flex-shrink-0 ring-4 ring-slate-200 dark:ring-slate-600">
              {p.avatar_url ? (
                ...
                <div className="w-full h-full flex items-center justify-center bg-orange-50 dark:bg-orange-900/30 text-orange-400">
                  <Users size={40} />
                </div>
              )}
            </div>
            ...
            <div className="flex-1 min-w-0">
              <h1 className="text-xl font-bold text-foreground">{p.nickname || p.eid}</h1>
              {p.username && <p className="text-sm text-slate-500">@{p.username}</p>}
            ...
            <button
              onClick={() => router.push('/dashboard/externalChannels/kuaishou')}
      ...
      <LookalikeSection platform="kuaishou" profileId={id} />
      ...
      <div>
        ...
        <div className="flex flex-wrap items-center gap-3 mb-4">
          <input
            ...
            placeholder="Tìm theo mô tả, hashtag..."
          ...
          >
            ...
            <option value="comments">Nhiều comments nhất</option>
          </select>
          {hasFilters && (
            <button
