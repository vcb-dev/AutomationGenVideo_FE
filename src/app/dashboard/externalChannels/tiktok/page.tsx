...
import { CircleNotch, FilmReel, Warning, MagnifyingGlassPlus, VideoCamera, UserCircle, Sparkle, Globe } from '@phosphor-icons/react';
...
export default function TiktokExternalPage() {
  ...
  // Load top keywords khi mount (cho onFocus)
  const [topKeywords, setTopKeywords] = useState<{ keyword: string; count: number }[]>([]);
  useEffect(() => {
    if (!token) return;
    scraperService.tiktokKeywordSuggest(token, '').then(setTopKeywords);
  }, [token]);
  ...
  useEffect(() => {
    ...
    suggestTimer.current = setTimeout(async () => {
      const results = await scraperService.tiktokKeywordSuggest(token, keyword);
  ...
  useEffect(() => {
    if (!token) return;
    scraperService.tiktokKeywordSuggest(token, '').then(list => {
  ...
  const videosQuery = useInfiniteQuery({
    queryKey: ['tiktok-videos', debouncedFilter, filterKeyword, minPlays, sortBy, dateFrom, dateTo],
    queryFn: ({ pageParam = 1 }) => {
      if (!token) return Promise.reject('No token');
      return scraperService.getTiktokVideos(token, {
  ...
  const profileToggleMutation = useMutation({
    ...
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['tiktok-profiles'] }),
  });
        ...
        <>
          ...
          {!videosQuery.isLoading && !videosQuery.isError && allVideos.length === 0 && (
            <div className="flex flex-col items-center py-16 gap-4 bg-card border border-border rounded-xl">
              ...
              <p className="text-sm text-foreground font-medium">Chưa có video TikTok nào</p>
              <p className="text-xs text-slate-400 text-center max-w-sm">Nhập keyword rồi bấm &quot;Tìm kiếm&quot; để cào video từ TikTok.</p>
            </div>
          )}
