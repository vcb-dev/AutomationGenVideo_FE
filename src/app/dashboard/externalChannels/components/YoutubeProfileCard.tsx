...
import { Users, VideoCamera, ArrowsClockwise, BookmarkSimple, Timer, CircleNotch, SealCheck, Buildings, UserCircle } from '@phosphor-icons/react';
import { YoutubeProfile } from '@/services/scraperService';
...
export default function YoutubeProfileCard({ profile: p, channelInfo, onScrape, onToggleBookmark, onToggleTracked, onViewDetail, onDelete, onEditClassification }: Props) {
    ...
    >
      ...
      <div className="flex items-center gap-3 p-3.5 pb-2">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1">
            <p className="text-sm font-semibold text-foreground truncate">{p.title || p.channel_id}</p>
      ...
      <div className="flex items-center gap-4 px-3.5 py-2">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-sm font-bold text-foreground">{formatNum(p.subscriber_count)}</span>
        </div>
        {p.video_count > 0 && (
          <div className="flex items-center gap-1.5">
            <VideoCamera size={13} className="text-red-500" />
            <span className="text-xs text-slate-500">{formatNum(p.video_count)} video</span>
          </div>
        )}
        {p.shorts_in_db > 0 && (
          <div className="flex items-center gap-1.5">
            <VideoCamera size={13} className="text-purple-500" />
            <span className="text-xs text-slate-500">{p.shorts_in_db}</span>
          </div>
        )}
      </div>
