...
export default function BilibiliExternalPage() {
  ...
  const deleteChannelMutation = useMutation({
    mutationFn: (id: number) => {
      ...
      return scraperService.deleteExternalChannel(token, 'bilibili', id);
    },
    onSuccess: (data) => {
  ...
  const searchMutation = useMutation({
    mutationFn: () => {
      ...
      return zh
        ? scraperService.bilibiliSearch(token, zh, num, vi)
        : scraperService.bilibiliSearch(token, vi, num);
    },
    onMutate: () => {
        ...
        <>
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 border border-border rounded-xl p-4">
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              {canManageChannels && (
                <SyncAllChannelsButton platform="bilibili" channelCount={profileTotal} />
              )}
            </div>
          </div>
            ...
            <div className="flex flex-col items-center py-16 gap-3 bg-card border border-border rounded-xl text-center">
              ...
              <p className="text-xs text-slate-400 max-w-sm">
                {profileTab === 'periodic'
                  ...
                  : 'Nhập Mid ở trên để bắt đầu cào.'}
              </p>
            </div>
          )}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              {profiles.map(p => (
                <BilibiliProfileCard
                  key={p.id}
                  profile={p}
                  onScrape={canManageChannels ? () => profileRescrape.mutate({ id: p.id, mid: p.mid, label: p.nickname || p.mid }) : undefined}
