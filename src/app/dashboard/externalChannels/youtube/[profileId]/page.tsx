...
import {
  ...
  FilmReel, SealCheck, Envelope, Globe, PaperPlaneTilt,
...
import { scraperService, YoutubeShort, YoutubeProfile } from '@/services/scraperService';
  ...
  const proposeMutation = useMutation({
    mutationFn: () => {
      return submit({
        ...
        video_url: short.url,
        author_username: profile?.channel_id || '',
...
export default function YoutubeProfileDetailPage() {
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="bg-card border border-border rounded-xl overflow-hidden">
        <div className="px-6 pb-6 pt-6">
          <div className="flex items-start gap-6">
            ...
            <button
              onClick={() => router.push('/dashboard/externalChannels/youtube')}
      ...
      <LookalikeSection platform="youtube" profileId={id} />
      ...
      <div>
        ...
        <div className="flex flex-wrap items-center gap-2.5 bg-card border border-border rounded-xl p-3 shadow-xs mb-4">
          ...
          <FilterSelect
            value={sortBy}
            onChange={val => setSortBy(val as 'scraped' | 'views')}
            options={[
        ...
        {allShorts.length > 0 && (
          <>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 2xl:grid-cols-7 gap-3">
              {allShorts.map(s => (
                <ShortCard key={s.video_id} short={s} profile={p} />
              ))}
