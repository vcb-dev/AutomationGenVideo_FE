      ...
      >
        ...
        <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent px-2.5 pb-2.5 pt-8">
          <div className="flex items-center gap-2.5 text-white text-xs">
            <span className="flex items-center gap-1"><Heart size={12} weight="fill" />{formatNum(video.digg_count)}</span>
            <span className="flex items-center gap-1"><ChatCircle size={12} weight="fill" />{formatNum(video.comment_count)}</span>
            {video.share_count > 0 && <span className="flex items-center gap-1"><ShareNetwork size={12} weight="fill" />{formatNum(video.share_count)}</span>}
          </div>
        </div>
...
export default function DouyinProfileDetailPage() {
  ...
  const [minDigg, setMinDigg] = useState('');
  ...
  const hasFilters = !!debouncedSearch || !!minDigg || sortBy !== 'date';
  const clearFilters = () => { setSearch(''); setMinDigg(''); setSortBy('date'); };

  // ─── Profile detail ───────────────────────────────────
  const detailQuery = useQuery({
    queryKey: ['douyin-profile-detail', id],
    queryFn: () => token ? scraperService.getDouyinProfileDetail(token, id) : Promise.reject(),
    enabled: !!token && !!id,
  ...
  const videosQuery = useInfiniteQuery({
    queryKey: ['douyin-profile-videos', id, debouncedSearch, minDigg, sortBy],
    queryFn: ({ pageParam = 1 }) => {
      if (!token) return Promise.reject();
      return scraperService.getDouyinProfileVideos(token, id, {
        page: pageParam,
        page_size: 24,
  ...
  const scrapeMutation = useMutation({
    mutationFn: () => {
      ...
      return scraperService.douyinProfileScrape(token, p.sec_user_id);
    },
    onSuccess: () => {
  ...
  const toggleMutation = useMutation({
    mutationFn: (field: 'is_bookmarked' | 'is_tracked') =>
      token ? scraperService.toggleDouyinProfile(token, id, field) : Promise.reject(),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['douyin-profile-detail', id] }),
  });
    ...
    <div className="flex flex-col gap-5">
        ...
        <div className="px-6 pb-6 pt-6">
          <div className="flex items-start gap-6">
            ...
            <div className="flex-1 min-w-0">
              ...
              {p.nickname && <p className="text-sm text-slate-500">@{p.username}</p>}
              ...
              <div className="flex items-center gap-4 mt-4 text-xs text-slate-500">
                ...
                {(p.total_diggs ?? 0) > 0 && (
                  <span className="flex items-center gap-1">
                    ...
                    {formatNum(p.total_diggs || 0)} tổng tim
                  </span>
                )}
            ...
            {/* Back button */}
            <button
              onClick={() => router.push('/dashboard/externalChannels/douyin')}
      ...
      <div>
        ...
        <div className="flex flex-wrap items-center gap-2.5 bg-card border border-border rounded-xl p-3 shadow-xs mb-4">
          ...
          <FilterSelect
            value={sortBy}
            onChange={val => setSortBy(val as 'date' | 'likes')}
            options={[
