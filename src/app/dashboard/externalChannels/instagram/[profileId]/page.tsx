  ...
  return (
    ...
    >
      <div className="relative aspect-[9/16] bg-slate-100 dark:bg-slate-800 overflow-hidden max-h-[320px]">
        ...
        {reel.is_paid_partnership && (
          <div className="absolute top-2 left-2 px-1.5 py-0.5 bg-amber-600/80 backdrop-blur-sm rounded text-white text-xs">
            Paid
          </div>
        )}
      </div>
...
export default function InstagramProfileDetailPage() {
  ...
  const detailQuery = useQuery({
    queryKey: ['instagram-profile-detail', id],
    queryFn: () => token ? scraperService.getInstagramProfileDetail(token, id) : Promise.reject(),
    enabled: !!token && !!id,
  ...
  const reelsQuery = useInfiniteQuery({
    queryKey: ['instagram-profile-reels', id, debouncedSearch, minPlays, sortBy],
    queryFn: ({ pageParam = 1 }) => {
      if (!token) return Promise.reject();
      return scraperService.getInstagramProfileReels(token, id, {
        page: pageParam,
        page_size: 24,
  ...
  const toggleMutation = useMutation({
    mutationFn: (field: 'is_bookmarked' | 'is_tracked') =>
      token ? scraperService.toggleInstagramProfile(token, id, field) : Promise.reject(),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['instagram-profile-detail', id] }),
  });
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-5 mt-4">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-sm text-slate-500">Bài viết</span>
                </div>
                <div className="flex items-center gap-1.5">
              ...
              <div className="flex items-center gap-4 mt-4 text-xs text-slate-500">
                ...
                {(p.total_likes ?? 0) > 0 && (
                  <span className="flex items-center gap-1">
                    ...
                    {formatNum(p.total_likes || 0)} tổng likes
                  </span>
                )}
      ...
      <div>
        ...
        {!reelsQuery.isLoading && allReels.length === 0 && (
          <div className="flex flex-col items-center py-16 gap-4 bg-card border border-border rounded-xl">
            ...
            <p className="text-sm text-foreground font-medium">Chưa có reel nào</p>
        ...
        {allReels.length > 0 && (
          <>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3">
              {allReels.map(r => (
                <InstagramReelCard key={r.post_id} reel={r} />
              ))}
            ...
            {!reelsQuery.hasNextPage && (
              <p className="text-xs text-slate-400 text-center py-4">Đã hiển thị toàn bộ {totalReels} reels.</p>
            )}
          </>
        )}
      </div>
