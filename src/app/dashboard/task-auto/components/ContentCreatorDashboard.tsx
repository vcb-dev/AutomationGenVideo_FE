...
import { FileText, Languages, Video, TrendingUp, ExternalLink, Target, Sparkles } from 'lucide-react'
...
import { CATEGORY, type Category } from './tokens'
...
function TargetBar({ label, value, target, category }: { label: string; value: number; target: number; category: Category }) {
  ...
  if (!target) {
    return (
      <div className="rounded-xl bg-slate-50 border border-slate-100 px-4 py-3.5">
        ...
        <p className="text-2xl font-extrabold text-slate-500">{value}</p>
  ...
  return (
    <div className={cn('rounded-xl border px-4 py-3.5', c.bg, c.border)}>
      <p className={cn('text-xs font-semibold uppercase tracking-wide mb-1.5', c.text)}>{label}</p>
      <div className="flex items-baseline gap-1.5 mb-2">
        <span className={cn('text-2xl font-extrabold', c.text)}>{value}</span>
...
export function ContentCreatorDashboard({ userId, from, to, teamName }: { userId: string; from: string; to: string; teamName?: string }) {
  ...
  return (
    <div className="space-y-5">
      {/* Greeting banner */}
      <div className="bg-gradient-to-br from-violet-600 to-indigo-600 rounded-2xl shadow-sm shadow-indigo-200/60 px-6 py-5 text-white">
        <div className="flex items-center gap-2 text-sm font-semibold text-violet-100">
          <Sparkles className="w-4 h-4" />
          Content Creator{teamName ? ` — ${teamName}` : ''}
        </div>
        <p className="text-lg font-bold mt-1 tracking-tight">Theo dõi content bạn sưu tầm, bản dịch và video được làm từ đó</p>
      </div>

      {/* Quick stats — theo kỳ đã chọn ở bộ lọc ngày */}
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <StatCard label="Content sưu tầm" value={period?.content_collected ?? 0} icon={FileText} tone="success" />
        <StatCard label="Bản dịch" value={period?.translations_count ?? 0} icon={Languages} tone="violet" />
        <StatCard label="Video được làm" value={period?.videos_made ?? 0} icon={Video} tone="brand" />
        <StatCard label="Lượt xem (Facebook)" value={totalViews} icon={TrendingUp} tone="info" />
      </div>
      ...
      >
        <div className="px-5 py-4 grid sm:grid-cols-2 gap-3">
          <TargetBar label="Content sưu tầm" value={monthRow?.content_collected ?? 0} target={target?.content_target ?? 0} category="content" />
          <TargetBar label="Bản dịch" value={monthRow?.translations_count ?? 0} target={target?.translation_target ?? 0} category="translation" />
        </div>
      </DashboardCard>
