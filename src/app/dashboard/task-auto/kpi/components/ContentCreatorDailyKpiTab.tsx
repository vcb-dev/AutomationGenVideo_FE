...
export function ContentCreatorDailyKpiTab({ canEdit, isLeader, userId, selectedTeamId, onTeamChange }: Props) {
  ...
  // Chỉ team có ít nhất 1 content creator mới cần set KPI ngày ở đây.
  const teamsWithContentCreator = (teams ?? []).filter(t => t.members?.some(m => m.is_content_creator))
  // Leader chỉ set được cho team mình lead (khớp quyền BE); admin/manager chọn team bất kỳ
  const selectableTeams = isLeader
    ? teamsWithContentCreator.filter(t => t.leader_id === userId)
    : teamsWithContentCreator
  ...
  const { data: savedKpis, isLoading } = useQuery({
    queryKey: ['task-auto', 'content-creator-daily-kpis', teamId, date],
    queryFn: () => getContentCreatorDailyKpis({ team_id: teamId, date }),
    enabled: !!teamId && !!date,
  })

  const team = teams?.find(t => t.id === teamId)
  const members = team?.members?.filter(m => m.is_content_creator) ?? []
  const savedByUser = new Map<string, ContentCreatorDailyKpi>(
  ...
  const saveMut = useMutation({
    mutationFn: () => upsertContentCreatorDailyKpis({
      team_id: teamId,
      date,
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-start gap-3 bg-amber-50 border border-amber-200 rounded-xl px-5 py-3.5 text-sm text-amber-700">
        ...
        <span>
          KPI ngày là <strong>số content mục tiêu của từng content creator cho đúng ngày đã chọn</strong>. Hiển thị
          làm &quot;KPI ngày&quot; trên card content creator ở dashboard leader. Để <strong>0</strong> = không set.
        </span>
      </div>
      ...
      <div className="flex flex-wrap items-end gap-3">
        ...
        {canEdit && (
          <button
            ...
            className="ml-auto flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl px-5 py-2.5 text-sm font-semibold transition-colors disabled:opacity-50"
          >
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-100">
              ...
              {!isLoading && (!teamId || members.length === 0) && (
                <tr>
                  <td colSpan={4}>
                    <EmptyState title={teamId ? 'Nhóm này chưa có content creator' : 'Chọn nhóm để set KPI ngày'} />
                  </td>
                </tr>
              )}
              {!isLoading && members.map(m => {
                ...
                return (
                  <tr key={m.user_id} className={cn(dirty && 'bg-emerald-50/40')}>
                    ...
                    <td className="px-5 py-3 text-right">
                      {canEdit ? (
                        <input
                          ...
                          placeholder="0"
                          className={cn(
                            'w-24 border rounded-lg px-2 py-1.5 text-right font-bold focus:outline-none focus:ring-2 focus:ring-emerald-400 transition-colors',
                            val > 0 ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-white border-gray-200 text-slate-400',
                          )}
                        />
                      ) : (
                        <span className={cn('text-sm font-bold', val > 0 ? 'text-emerald-700' : 'text-slate-300')}>
                          {val > 0 ? val : '—'}
                        </span>
                      )}
                    </td>
